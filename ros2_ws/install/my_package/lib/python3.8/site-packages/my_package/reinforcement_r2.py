...
class ObjectDetectionNode(Node):
    ...
    def timer_callback(self):
        ...
        while not done:
            # Publish observation
            observation_json = {
                "silo_states": observation[:5].tolist(),  # First 5 elements are silo states
                "ball_color": int(observation[5]),        # Next element is ball color
                "team_color": int(observation[6])         # Next element is team color
            }
            msg = String()
